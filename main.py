...
async def analyze_image_with_groq(image_bytes: bytes, user_id: int = None) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        detail_level = user_detail_levels.get(user_id, 'standard') if user_id else 'standard'
...
Описание должно быть коротким (2-3 предложения), но понятным."""
            ...
            user_prompt = "Опиши это изображение кратко, выделив самое главное."
...
async def main():
    ...
    async def handle_start(message: types.Message):
        ...
        await message.reply(
            ...
            "• Когда кто-то отправляет изображение — я анализирую его и озвучиваю описание\n\n"
            ...
            "• `/image_detail_level [уровень]` - настроить детализацию описания\n"
            "• `/my_image_settings` - проверить текущие настройки\n"
            "• `/reset_image_settings` - сбросить к стандартным\n\n"
            "**Для администратора:**\n"
    ...
    async def handle_image_detail_level(message: types.Message):
        ...
        try:
            ...
            if not message.reply_to_message:
                await message.reply(
                    "📋 **Настройка детализации описания изображений**\n\n"
                    "**Доступные уровни:**\n"
                    "• **Краткий** - основные объекты и действия\n"
                    "• **Стандартный** - детальное описание для незрячих (по умолчанию)\n"
                    "• **Технический** - максимально подробное описание\n\n"
                    "**Использование:**\n"
                    "Ответьте на сообщение командой `/image_detail_level` и выберите уровень:\n"
                    "• `/image_detail_level brief` - краткий\n"
                    "• `/image_detail_level standard` - стандартный\n"
                    "• `/image_detail_level technical` - технический\n\n"
                    "**Текущий уровень:** Стандартный (рекомендуется для незрячих)",
                    parse_mode="Markdown"
                )
                return
            ...
            await message.reply(
                f"✅ Уровень детализации изменен на: **{level_names.get(level, 'Неизвестный')}**\n\n"
                f"Теперь все изображения будут описываться с выбранным уровнем детализации.",
                parse_mode="Markdown"
            )
            
        except Exception as e:
            logger.error(f"Ошибка при настройке детализации: {e}")
            await message.reply(f"❌ Ошибка при настройке: {str(e)}")
    ...
    async def handle_my_image_settings(message: types.Message):
        ...
        try:
            user_id = message.from_user.id
            current_level = user_detail_levels.get(user_id, 'standard')
            
            level_descriptions = {
                'brief': {
                    'name': 'Краткий',
                    'description': 'Основные объекты и действия (2-3 предложения)',
                    'best_for': 'Быстрый обзор изображений'
                },
                'standard': {
                    'name': 'Стандартный',
                    'description': 'Детальное описание для незрячих (рекомендуется)',
                    'best_for': 'Полное понимание изображения'
                },
                'technical': {
                    'name': 'Технический',
                    'description': 'Максимально подробный технический анализ',
                    'best_for': 'Детальное изучение всех аспектов'
                }
            }
            
            current = level_descriptions[current_level]
            
            settings_text = (
                f"🔧 **Ваши настройки описания изображений**\n\n"
                f"**Текущий уровень:** {current['name']}\n"
                f"**Описание:** {current['description']}\n"
                f"**Лучше всего подходит для:** {current['best_for']}\n\n"
                f"**Изменить уровень:**\n"
                f"• `/image_detail_level brief` - краткий\n"
                f"• `/image_detail_level standard` - стандартный\n"
                f"• `/image_detail_level technical` - технический\n\n"
                f"**Для незрячих пользователей рекомендуется:** Стандартный уровень"
            )
            ...
            await message.reply(f"❌ Ошибка при получении настроек: {str(e)}")
    ...
    async def handle_reset_image_settings(message: types.Message):
        ...
        try:
            ...
            # Удаляем настройки пользователя (возвращаемся к стандартным)
            if user_id in user_detail_levels:
                ...
                await message.reply(
                    ...
                    "✅ Теперь используется стандартный уровень детализации\n"
                    "📋 **Стандартный уровень включает:**\n"
                    "• Детальное описание для незрячих\n"
                    "• Расположение объектов (центр, слева, справа)\n"
                    "• Размеры и расстояния\n"
                    "• Цвета и текстуры\n"
                    "• Текст и символы\n"
                    "• Эмоции и настроение\n\n"
                    "**Проверить настройки:** `/my_image_settings`\n"
                    "**Изменить уровень:** `/image_detail_level [уровень]`",
                    parse_mode="Markdown"
                )
            else:
                await message.reply(
                    "ℹ️ **У вас уже стандартные настройки**\n\n"
                    "Настройки не были изменены, используется стандартный уровень детализации.",
                    parse_mode="Markdown"
                )
            
        except Exception as e:
            logger.error(f"Ошибка при сбросе настроек: {e}")
            await message.reply(f"❌ Ошибка при сбросе настроек: {str(e)}")
