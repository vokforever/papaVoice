...
class UserBlockManager:
    ...
    В личных чатах (private):
    - Все функции доступны для всех пользователей (блокировки не применяются)
    
    В групповых чатах (group, supergroup):
    - S2T (распознавание голоса в текст): ЗАБЛОКИРОВАНО по умолчанию для ADMIN
    - T2S (озвучивание текста): РАЗБЛОКИРОВАНО по умолчанию для всех
    ...
    def is_user_blocked_s2t(self, user_id: int, chat_type: str = 'private') -> bool:
        ...
        Returns:
            bool: True если заблокирован, False если разрешено
        """
        # В личных чатах блокировки не применяются - все функции доступны
        if chat_type == 'private':
            return False
            
        # В групповых чатах применяем блокировки
        if user_id not in self.blocked_users:
            # Для ADMIN по умолчанию блокируем S2T (распознавание голоса) только в группах
            if user_id == ADMIN_ID:
                return True
            return False
        return self.blocked_users[user_id].block_s2t
    
    def is_user_blocked_t2s(self, user_id: int, chat_type: str = 'private') -> bool:
...
- Ключевая функциональная информация (текст, числа, действия)

Опиши изображение коротко, 1-2 предложения, чтобы описание было понятным и емким, исключая лишние детали про цвета, стиль и общую атмосферу."""
            
            user_prompt = "Опиши это изображение кратко, фокусируясь на основных объектах и функциональной информации. Опусти детали про цвета и дизайн."
...
async def main():
    ...
    async def handle_voice(message: types.Message):
        ...
        if user_block_manager.is_user_blocked_s2t(user_id, message.chat.type):
            if user_id == ADMIN_ID:
                logger.info(f"ADMIN {user_id} заблокирован для S2T в {message.chat.type} чате - голосовое сообщение игнорируется")
            else:
                logger.info(f"Пользователь {user_id} заблокирован для S2T в {message.chat.type} чате - сообщение игнорируется")
            return
    ...
    async def handle_audio(message: types.Message):
        ...
        if user_block_manager.is_user_blocked_s2t(user_id, message.chat.type):
            if user_id == ADMIN_ID:
                logger.info(f"ADMIN {user_id} заблокирован для S2T в {message.chat.type} чате - аудио файл игнорируется")
            else:
                logger.info(f"Пользователь {user_id} заблокирован для S2T в {message.chat.type} чате - аудио файл игнорируется")
            return
    ...
    async def handle_video_note(message: types.Message):
        ...
        if user_block_manager.is_user_blocked_s2t(user_id, message.chat.type):
            if user_id == ADMIN_ID:
                logger.info(f"ADMIN {user_id} заблокирован для S2T в {message.chat.type} чате - видео сообщение игнорируется")
            else:
                logger.info(f"Пользователь {user_id} заблокирован для S2T в {message.chat.type} чате - видео сообщение игнорируется")
            return
    ...
    async def handle_document(message: types.Message):
        ...
        if user_block_manager.is_user_blocked_s2t(user_id, message.chat.type):
            if user_id == ADMIN_ID:
                logger.info(f"ADMIN {user_id} заблокирован для S2T в {message.chat.type} чате - аудио документ игнорируется")
            else:
                logger.info(f"Пользователь {user_id} заблокирован для S2T в {message.chat.type} чате - аудио документ игнорируется")
            return
    ...
    async def handle_text(message: types.Message):
        ...
        if user_id == ADMIN_ID:
            ...
            if user_block_manager.is_user_blocked_t2s(user_id, message.chat.type):
                await message.reply("🚫 У вас заблокировано озвучивание текста. Проверьте настройки блокировки.")
                logger.info(f"Администратор {user_id} заблокирован для T2S в {message.chat.type} чате")
                return
